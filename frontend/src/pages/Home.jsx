      ...
      {particles.map(p => (
        <div
          ...
          style={{
            ...
            boxShadow: `0 0 ${p.size * 2}px rgba(251,191,36,${p.opacity * 0.5})`,
...
const RwandaHills = () => (
  <div className="absolute bottom-0 left-0 right-0 pointer-events-none" aria-hidden="true">
    <svg viewBox="0 0 1440 200" className="w-full h-auto" preserveAspectRatio="none">
      {/* Far hills */}
      <path
        d="M0,160 C120,100 240,80 360,110 C480,140 540,60 720,90 C900,120 1020,50 1140,80 C1260,110 1380,70 1440,100 L1440,200 L0,200Z"
        fill="rgba(15,23,42,0.3)"
        className="animate-drift-slow"
      />
      {/* Mid hills */}
      <path
        d="M0,170 C180,120 300,140 480,130 C660,120 780,160 960,140 C1140,120 1320,150 1440,135 L1440,200 L0,200Z"
        fill="rgba(15,23,42,0.5)"
      />
      {/* Near hills */}
      <path
        d="M0,180 C200,160 400,175 600,165 C800,155 1000,170 1200,160 C1350,155 1440,165 1440,170 L1440,200 L0,200Z"
        fill="rgb(15,23,42)"
      />
    </svg>
  </div>
    ...
    <svg className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <pattern id="imigongo-hero" x="0" y="0" width="60" height="60" patternUnits="userSpaceOnUse">
          <polygon points="30,0 60,30 30,60 0,30" fill="none" stroke="rgba(251,191,36,1)" strokeWidth="0.5" />
          <polygon points="30,10 50,30 30,50 10,30" fill="none" stroke="rgba(251,191,36,0.6)" strokeWidth="0.3" />
          <circle cx="30" cy="30" r="2" fill="rgba(251,191,36,0.4)" />
        </pattern>
      </defs>
...
const HolographicOverlay = () => (
  <div className="absolute inset-0 pointer-events-none overflow-hidden opacity-[0.06]" aria-hidden="true">
    ...
    {[0, 1, 2].map(i => (
      <div
        key={i}
        className="absolute left-0 right-0 h-px bg-gradient-to-r from-transparent via-amber-400 to-transparent animate-scan-sweep"
        style={{ top: `${25 + i * 25}%`, animationDelay: `${i * 1}s` }}
      />
    ))}
    {/* Corner nodes */}
    {[[10, 15], [85, 20], [15, 75], [80, 80]].map(([x, y], i) => (
      <div
        key={`node-${i}`}
        className="absolute w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse-glow"
