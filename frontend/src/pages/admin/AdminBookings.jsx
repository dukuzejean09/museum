...
const typeStyles = {
  physical: { bg: 'bg-amber-900/20 text-amber-400', icon: MapPin, label: 'In-Person' },
  online: { bg: 'bg-emerald-900/20 text-emerald-400', icon: Globe, label: 'Online' },
};
    ...
    <div>
          ...
          {[
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-xs font-medium transition ${
                filter === f.key ? 'bg-slate-900 shadow text-slate-200' : 'text-slate-400 hover:text-slate-300'
              }`}
            >
              {f.label}
            </button>
            ...
            <table className="w-full min-w-[700px] text-sm">
              ...
              <tbody className="divide-y divide-slate-700">
                {bookings.map(b => {
                  ...
                  return (
                    <tr key={b._id} className="hover:bg-slate-800">
                      ...
                      <td className="px-4 py-3">
                        <div>
                          ...
                          <p className="text-xs text-slate-400">{b.visitorEmail}</p>
                          {b.visitorPhone && <p className="text-xs text-slate-400">{b.visitorPhone}</p>}
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <p className="text-slate-200">{new Date(b.date).toLocaleDateString()}</p>
                        {b.time && <p className="text-xs text-slate-400">{b.time}</p>}
                      </td>
                      <td className="px-4 py-3 text-slate-300">{b.groupSize}</td>
                      <td className="px-4 py-3">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${statusStyles[b.status] || 'bg-slate-800'}`}>
                          {b.status}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        {b.accessCodeId ? (
                          <span className="inline-flex items-center gap-1 text-xs font-mono bg-green-900/20 text-green-400 px-2 py-1 rounded-lg">
                        ...
                        <div className="flex items-center gap-1">
                          {b.status === 'pending' && (
                            <>
                              <button
                                ...
                                className="p-1.5 rounded-lg text-green-600 hover:bg-green-900/20"
                              ...
                              <button
                                ...
                                className="p-1.5 rounded-lg text-red-600 hover:bg-red-900/20"
                                title="Reject"
                              >
                                <XCircle size={18} />
                              </button>
                          ...
                          {!b.accessCodeId && b.status !== 'rejected' && b.status !== 'cancelled' && (
                            <button
                              ...
                              className="p-1.5 rounded-lg text-amber-600 hover:bg-amber-900/20"
                          ...
                          {isAdmin && (
                            <button
                              ...
                              className="p-1.5 rounded-lg text-slate-400 hover:bg-slate-800 hover:text-red-600"
                              title="Delete"
                            >
                              <Trash2 size={18} />
                            </button>
      ...
      {bookings.filter(b => b.message).length > 0 && (
        <div className="mt-6 card p-6">
          <h2 className="text-lg font-semibold text-slate-300 mb-4">Special Requests & Messages</h2>
          <div className="space-y-3">
            {bookings.filter(b => b.message).map(b => (
              <div key={b._id} className="border border-slate-700 rounded-lg p-3">
                <div className="flex items-center gap-2 mb-1">
                  <p className="text-sm font-medium text-slate-200">{b.visitorName}</p>
                ...
                <p className="text-sm text-slate-400">{b.message}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {confirmModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setConfirmModal(null)}>
          <div className="bg-slate-900 rounded-2xl shadow-2xl max-w-md w-full p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-slate-200">Confirm Booking</h3>
              <button onClick={() => setConfirmModal(null)} className="p-1 rounded-lg hover:bg-slate-800">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5 mb-2">
                  {[
                    ...
                    <button
                      ...
                      className="px-2.5 py-1.5 rounded-lg text-xs font-medium bg-slate-800 text-slate-400 hover:bg-slate-700 transition"
                    >
                      {opt.label}
                    </button>
                  ))}
                ...
                <input
                  ...
                  className="w-full px-3 py-2 text-sm border border-slate-700 rounded-xl bg-slate-800 text-slate-200 focus:ring-2 focus:ring-amber-500 outline-none"
                />
      ...
      {accessModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setAccessModal(null)}>
          <div className="bg-slate-900 rounded-2xl shadow-2xl max-w-sm w-full p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-slate-200">Access Code Generated</h3>
              <button onClick={() => setAccessModal(null)} className="p-1 rounded-lg hover:bg-slate-800">
                <X size={20} className="text-slate-400" />
              </button>
            </div>
            ...
            <div className="bg-slate-800 rounded-xl p-3 mb-3">
              ...
              <div className="flex items-center justify-between">
                <span className="font-mono font-bold text-lg text-slate-200">{accessModal.code}</span>
                <button onClick={() => copyToClipboard(accessModal.code)} className="p-1.5 rounded-lg hover:bg-slate-700" title="Copy">
            ...
            <div className="bg-slate-800 rounded-xl p-3 mb-4">
              <p className="text-xs text-slate-400 mb-1">Gateway Link</p>
              <div className="flex items-center justify-between gap-2">
                <span className="text-xs text-blue-600 truncate">{accessModal.gatewayUrl}</span>
                <button onClick={() => copyToClipboard(accessModal.gatewayUrl)} className="p-1.5 rounded-lg hover:bg-slate-700 flex-shrink-0" title="Copy">
                  <Copy size={16} className="text-slate-400" />
                </button>
              </div>
            </div>
