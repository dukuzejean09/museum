...
const AdminMessages = () => {
 ...
 {messages.map((msg) => (
 <div key={msg._id} className="bg-slate-900 rounded-2xl shadow-sm border border-slate-700 overflow-hidden">
 {/* Header */}
 <div className="p-5">
 ...
 className="text-xs border border-slate-700 rounded-lg px-2 py-1 bg-slate-800"
 >
 ...
 className="p-1.5 text-red-500 hover:bg-red-900/20 rounded-lg transition" title="Delete">
 <Trash2 size={16} />
 </button>
 )}
 </div>
 ...
 className="w-full px-5 py-2 text-sm text-slate-400 hover:bg-slate-800/50 flex items-center gap-1"
 >
 ...
 <p className="text-sm text-slate-300 mt-1">{reply.message}</p>
 </div>
 ))}
 </div>
 )}
 ...
 className="flex-1 px-4 py-2 border border-slate-700 rounded-xl bg-slate-800 text-sm focus:ring-2 focus:ring-amber-500 outline-none"
 />
 <button
