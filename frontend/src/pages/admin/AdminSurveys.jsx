...
const AdminSurveys = () => {
  ...
  const renderStars = (rating) => (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map(s => (
        <Star key={s} size={14} className={s <= rating ? 'fill-amber-400 text-amber-400' : 'text-slate-300'} />
      ))}
    </div>
  );
    ...
    <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-slate-900 rounded-xl shadow-sm p-5 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-slate-200">{stats.avgRating}/5</p>
            </div>
          </div>
          <div className="bg-slate-900 rounded-xl shadow-sm p-5 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-slate-200">{stats.recommendRate}%</p>
            </div>
          </div>
          <div className="bg-slate-900 rounded-xl shadow-sm p-5 flex items-center gap-4">
            <div className="bg-purple-500 text-white p-3 rounded-lg"><Clock size={24} /></div>
            <div>
              <p className="text-sm text-slate-400">Most Common Duration</p>
      ...
      {stats && stats.total > 0 && (
        <div className="bg-slate-900 rounded-xl shadow-sm p-6 mb-8">
          <h2 className="text-lg font-semibold text-slate-300 mb-4">Rating Distribution</h2>
            ...
            {[5, 4, 3, 2, 1].map(rating => {
              ...
              return (
                <div key={rating} className="flex items-center gap-3">
                  <span className="text-sm text-slate-400 w-6">{rating}★</span>
                  <div className="flex-1 bg-slate-800 rounded-full h-4 overflow-hidden">
                    <div className="bg-amber-500 h-full rounded-full transition-all" style={{ width: `${pct}%` }} />
                  </div>
                  <span className="text-sm text-slate-400 w-16 text-right">{count} ({Math.round(pct)}%)</span>
                </div>
              );
            })}
          </div>
      ...
      {/* Survey List */}
      {surveys.length === 0 ? (
        <div className="bg-slate-900 rounded-xl shadow-sm p-8 text-center text-slate-400">
          <BarChart3 size={48} className="mx-auto mb-3 opacity-50" />
          <p>No survey responses yet.</p>
        </div>
      ) : (
        <div className="bg-slate-900 rounded-xl shadow-sm p-6">
          <h2 className="text-lg font-semibold text-slate-300 mb-4">Individual Responses</h2>
          <div className="space-y-4">
            {surveys.map(s => (
              <div key={s._id} className="border border-slate-700 rounded-lg p-4 hover:bg-slate-800">
                <div className="flex items-start justify-between">
                  <div>
                    <div className="flex items-center gap-3 mb-1">
                      <p className="font-medium text-slate-200">{s.visitorName || 'Anonymous'}</p>
                      {renderStars(s.overallRating)}
                    </div>
                    <p className="text-xs text-slate-400">{new Date(s.createdAt).toLocaleDateString()}</p>
                  </div>
                  <button onClick={() => handleDelete(s._id)} className="text-slate-400 hover:text-red-600">
                    <Trash2 size={16} />
                  </button>
                </div>
                ...
                {s.comments && <p className="mt-2 text-sm text-slate-400">{s.comments}</p>}
              </div>
            ))}
          </div>
        </div>
